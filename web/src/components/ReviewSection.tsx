    ...
    <div className="bg-white rounded-2xl shadow-md ring-1 ring-gray-900/5 p-4 md:p-5">
      ...
      {!isLoading && !error && (
        <ul className="space-y-2 mb-4 text-sm">
          ...
          {(data || []).map((r: any) => (
            <li key={r.id} className="border rounded-2xl p-2.5 flex justify-between items-start gap-2 border-slate-200 hover:bg-slate-50 transition-colors">
              <div>
      ...
      {user ? (
        ...
        >
          ...
          />
          <button
            className="self-start px-3 py-2 rounded-2xl border text-xs sm:text-sm font-medium text-[#0F766E] border-[#0F766E] bg-white hover:bg-teal-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm hover:shadow-md"
            type="submit"
